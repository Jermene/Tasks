...
// +++ #84 Vla79  18.06.2018
//Выполняет подключение к почтовому ящику и получает список папок
//
// Параметры:
// УчетнаяЗапись - СправочникСсылка.УчетныеЗаписиЭлектроннойПочты - учетная запись,
//					из которой необходимо получить список папок.
//
// Возвращаемое значение:
//  СписокПапок - СписокЗначений - список папок, которые есть в учетной записи IMAP:
//  
Функция узПолучитьСписокПапокIMAP(Знач УчетнаяЗапись) Экспорт
