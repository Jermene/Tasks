...
Функция ЗагрузитьИзмененияИзХранилища(АдресФайлаПолученныйНаКлиенте = Неопределено,ФайлВыгрузкиИзменений = Неопределено) Экспорт
	...
	Для каждого СтрокаТЗИсторияХранилища из ТЗИсторияХранилища цикл
		...
		Если НЕ ЗначениеЗаполнено(СтрокаТЗИсторияХранилища.Задача) 
...
Процедура ОтправитьУведомлениеОшибкаВКомментарииХранилища(ТекстОшибки,СтрокаТЗИсторияХранилища)
	...
	//+ #97 Дзеса Ігор	
	Если мМассивПользователейКомуОтправлятьСистемныеУведомления = Неопределено Тогда
		ВызватьИсключение НСтр("ru=' В настройках системы не заполнен список пользователей, которым нужно отправлять системные уведомления!'");
	КонецЕсли;
	//- #97 Дзеса Ігор	
